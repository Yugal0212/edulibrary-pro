...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedRole === "patron") {
      router.push("/auth/success?role=patron&status=approved")
    } else if (selectedRole === "librarian") {
      router.push("/auth/success?role=librarian&status=pending")
    }
  }
          ...
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <Button
                ...
                disabled={isLoading}
              >
                {isLoading ? "Creating Account..." : `Create ${currentRole.title} Account`}
              </Button>
            </form>
